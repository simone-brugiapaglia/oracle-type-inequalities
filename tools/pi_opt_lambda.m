function [pi, L]=pi_opt_lambda(n,d,options,level,sparsities)
%% Computes the drawing probability minimizing the lambda quantity, i.e.
% pi_k proportional to a uniform bound on || B_{k,S} B_{k,S}^* ||_{2 -> 2}
...
for i=1:n
        ...
        for j=1:length(ilevel)-1
            norm_FW(j) = max(abs(FW(ilevel(j)+1:ilevel(j+1))))^2 ;
        end
        vect(i)=0;
        Pi(i)=sum(norm_FW.*sparsities');
end
Pi=fftshift(Pi);
